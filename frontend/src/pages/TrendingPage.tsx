...
export const TrendingPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="py-12 text-center text-xs text-[#888]">Analyzing platform-wide signals...</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {trends.map((t) => (
            <div
              key={t.id}
              className="rounded-sm bg-[#0A0A0A] border border-[#222] hover:border-amber-500/40 transition-all p-5 flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="text-sm font-semibold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
              ...
              <div className="mt-4 pt-3 border-t border-[#222] space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-1.5 pt-1">
                  <span className="text-[10px] text-[#666]">Related Hubs:</span>
                  {t.related_communities?.map((comm, idx) => (
                    <Link
                      key={idx}
                      to={`/c/${comm}`}
                      className="rounded bg-[#050505] text-[#EDEDED] hover:text-white border border-[#222] px-1.5 py-0.5 text-[10px] font-semibold transition-colors"
                    >
                      /c/{comm}
                    </Link>
                  ))}

...
export const UserProfilePage: React.FC = () => {
  ...
  if (isLoading) {
    return <div className="py-12 text-center text-xs text-gray-400">Loading user profile...</div>;
  }

  if (!profile) {
    return <div className="text-center py-12 text-xs text-gray-400">User not found.</div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-3xl bg-[#111827] border border-[#1F2937] p-6 space-y-4 shadow-xl">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <img
              src={profile.avatarUrl || `https://api.dicebear.com/7.x/bottts/svg?seed=${profile.username}`}
              alt="Avatar"
              className="h-20 w-20 rounded-2xl bg-indigo-950 border-2 border-indigo-500/40 object-cover shadow-lg"
            />
            <div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 rounded-2xl bg-[#0B0F19] border border-[#1F2937] px-4 py-2">
              <Flame className="h-5 w-5 text-amber-400" />
              <div>
                <div className="text-xs font-black text-white">{profile.reputation}</div>
                <div className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold">Reputation</div>
              </div>
            </div>
            ...
            {isSelf && (
              <button
                ...
                className="flex items-center gap-1.5 rounded-2xl bg-[#1F2937] hover:bg-gray-700 text-gray-200 px-3.5 py-2 text-xs font-semibold border border-gray-700 transition-colors shadow-sm"
              >
        ...
        {/* Bio */}
        {profile.bio && (
          <p className="text-xs text-gray-300 leading-relaxed bg-[#0B0F19] rounded-xl p-3 border border-[#1F2937]">
            {profile.bio}
          </p>
        )}
            ...
            <div className="flex flex-wrap gap-1.5">
              {profile.skills?.length > 0 ? (
                profile.skills.map((sk: string, i: number) => (
                  <span key={i} className="rounded-lg bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 px-2 py-0.5 text-xs font-semibold">
                    {sk}
                  </span>
                ))
              ) : (
            ...
            <div className="flex flex-wrap gap-1.5">
              {profile.interests?.length > 0 ? (
                profile.interests.map((int: string, i: number) => (
                  <span key={i} className="rounded-lg bg-purple-500/10 text-purple-300 border border-purple-500/20 px-2 py-0.5 text-xs font-semibold">
                    {int}
                  </span>
                ))
              ) : (
                <span className="text-xs text-gray-500 italic">No interests added yet</span>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="pt-3 border-t border-[#1F2937]">
            ...
            <div className="flex flex-wrap gap-2">
              {profile.badges.map((b: any, idx: number) => (
                <div
                  key={idx}
                  className="rounded-xl bg-amber-500/10 border border-amber-500/30 px-3 py-1.5 text-xs text-amber-300 flex items-center gap-1.5"
                  title={b.description}
                >
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 border-b border-[#1F2937] pb-2 text-xs font-bold">
          <button
            onClick={() => setActiveTab('posts')}
            className={`pb-2 px-1 border-b-2 transition-colors ${
              activeTab === 'posts' ? 'border-indigo-500 text-white' : 'border-transparent text-gray-400 hover:text-white'
            }`}
          >
          ...
          {isSelf && (
            <button
              ...
              className={`pb-2 px-1 border-b-2 transition-colors ${
                activeTab === 'saved' ? 'border-indigo-500 text-white' : 'border-transparent text-gray-400 hover:text-white'
              }`}
            >
            ...
            {profile.recentPosts?.length === 0 ? (
              <div className="rounded-2xl bg-[#111827] border border-[#1F2937] p-8 text-center space-y-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-500 mt-0.5">Start an institutional thread or recruit a team for your prototype.</p>
                </div>
                {isSelf && (
                  <Link
                    to="/create"
                    className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-xs font-bold text-white shadow-lg shadow-indigo-600/20 transition-all"
                  >
              ...
              profile.recentPosts.map((p: any) => (
                <Link
                  key={p.id}
                  to={`/post/${p.id}`}
                  className="block rounded-2xl bg-[#111827] border border-[#1F2937] p-4 hover:border-indigo-500/40 transition-all"
                >
                  <div className="flex items-center justify-between text-[11px] text-gray-400 mb-1">
                    <span className="font-semibold text-indigo-400">/c/{p.community_slug}</span>
            ...
            {savedPosts.length === 0 ? (
              <div className="rounded-2xl bg-[#111827] border border-[#1F2937] p-8 text-center space-y-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-500 mt-0.5">Bookmark useful technical solutions, research threads, or campus notices.</p>
                </div>
                <Link
                  to="/home"
                  className="inline-flex items-center gap-1.5 rounded-xl bg-[#1F2937] hover:bg-gray-700 px-4 py-2 text-xs font-bold text-gray-200 transition-all"
                >
              ...
              savedPosts.map((sp: any) => (
                <Link
                  ...
                  className="block rounded-2xl bg-[#111827] border border-[#1F2937] p-4 hover:border-indigo-500/40 transition-all"
                >
                  <div className="text-[11px] text-indigo-400 font-semibold mb-1">/c/{sp.community_slug}</div>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-3xl bg-[#111827] border border-[#1F2937] p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-[#1F2937]">
              <div>
                <h3 className="text-base font-bold text-white">Edit SIT Profile</h3>
                <p className="text-[11px] text-gray-400">Update your verified academic department, skills, and bio</p>
              </div>
              <button
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4 text-xs">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 rounded-xl bg-[#0B0F19] border border-[#1F2937] p-2.5 text-white focus:outline-none focus:border-indigo-500"
                  />
                  <button
                    type="button"
                    onClick={() => handleAddSkill()}
                    className="rounded-xl bg-[#1F2937] hover:bg-gray-700 px-3 py-2 text-white font-semibold"
                  >
                    Add
                  </button>
                </div>
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {editSkills.map((sk) => (
                    <span
                      key={sk}
                      className="rounded-lg bg-indigo-500/15 text-indigo-300 border border-indigo-500/30 px-2 py-0.5 text-xs flex items-center gap-1"
                    >
                      <span>{sk}</span>
                      <button
                        type="button"
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {editInterests.map((int) => (
                    <span
                      key={int}
                      className="rounded-lg bg-purple-500/15 text-purple-300 border border-purple-500/30 px-2 py-0.5 text-xs flex items-center gap-1"
                    >
                      <span>{int}</span>
                      <button
                        type="button"
              ...
              <div className="pt-3 flex justify-end gap-2 border-t border-[#1F2937]">
                <button
                  type="button"
                  onClick={() => setIsEditModalOpen(false)}
                ...
                <button
                  type="submit"
                  disabled={isSaving}
                  className="rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 px-5 py-2 font-bold text-white shadow-lg shadow-indigo-600/30"
                >
